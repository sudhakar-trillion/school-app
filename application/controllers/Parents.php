	 ...
	 public function __construct()
		{
			...
			$this->studentDetails = $this->schedulinglib->getstudemtSessiondetails();
...
//vieweditchildprofile starts here
	...
	public function vieweditchildprofile()
	{
		...
		$cond['StudentId'] = $this->session->userdata('StudentId');
		$table = 'students';
		$fields='Student, LastName, BloodGroup,ProfileIPic,DOB';
...
//vieweditchildprofile ends hree
	...
	public function showhomeworks()
	{
		...
		$cond['hw.ClassSlno'] 		=	$studentDetails['classid'];
		$cond['hw.ClassSection'] 	=	$studentDetails['sectionId'];
		$cond['hw.StudentId'] 		=	$studentDetails['studentId'];
		$cond['hw.Status'] 		=	'Assigned';
		
		
		
		
		$baseurl='student-home-works';
		$perpage=10;
		$order_by_field='hw.HomeworkId';
		$datastring='homeworks';
		$pagination_string = 'pagination_string';
		
		$data= $this->tsmpaginate->getHomeworks($table,$cond,$baseurl,$perpage,$order_by_field,$datastring,$pagination_string);
		#print_r($data); exit;
		...
		if($data['homeworks']!='0')
		{
			...
			$this->load->view('Parent/show-home-works',$data);
		}
		else
		{
			$data['route']='';
...
public function feepaid()
{
	...
	*/
	...
	else 
	{
		...
		redirect(base_url('parent/payment-history'));
}
...
/*
		$table1='feecollection as coll';
		$table2='feetranasactiondetails as det';
		$fetchingfields='MonthName(coll.Month) as Month, coll.AcademicYear, coll.ActualFee, coll.Paid, coll.Due, date_format(coll.PaidOn,"%d-%m-%Y") as PaidOn, det.CardNo, det.TransactionId, coll.StudentId';
		$joinedOn='det.FeeCollectionId=coll.FeeCollectionId';
		$joinType='';
		$cond='';
...
*/
		...
		$cond['std.StudentId']	=	$studentDetails['studentId'];
		$cond['std.ClassName']	=	$studentDetails['classid'];
		$cond['std.ClassSection']	=	$studentDetails['sectionId'];
		...
		foreach( $academics as $key=>$val )
			...
			if( $mnth<=date('m') )
			{
				...
				foreach($stddetails->result() as $stds)
				{
					...
					$cond['coll.StudentId'] = $studentDetails['studentId'];
					$cond['coll.ClassId'] = $stds->ClassId; 
					$cond['coll.SectionId'] = $stds->Secid;  
					$cond['Month(coll.Month)'] = $mnth; 
					$cond['coll.AcademicYear'] = $AcademicYear;
					
					
					$this->db->select('sum(coll.Paid) as Paid, det.CardNo, date_format(coll.PaidOn,"%d-%m-%Y") as PaidOn, det.TransactionId');
					$this->db->from('feecollection as coll');
					$this->db->join('feetranasactiondetails as det','det.FeeCollectionId=coll.FeeCollectionId');
					$this->db->where($cond);
					
					$qry = $this->db->get();
					foreach( $qry->result() as $det)
					{
						if( $det->Paid!='')	
						{
							$paidAmount = $det->Paid;
		...
		$stddetails = array_reverse($output_arrr);
...
*/		
		if($stddetails!='0')
		{
			$data['details'] = $stddetails;

			$this->load->view('Parent/paymenthistory',$data);
		}
		else
		{
			$data['routeto'] = 'parent-dashboard';
