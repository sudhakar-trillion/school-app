...
//addnonteachingdetails ends here
	...
	public function viewnonteachingdetails()
	{
		$this->load->view(HEADER);
		
		$cond=array();

		$table = 'staffdetails AS sfd';
		$baseurl='view-nonteaching-staff-details';
		$perpage=20;
		$order_by_field='StaffDetailId';
		$datastring='nontexhingstaffDetails';
		$pagination_string = 'pagination_string';
		
		$qery = $this->tsmpaginate->getnonteachingPagination($table,$cond,$baseurl,$perpage,$order_by_field='',$datastring,$pagination_string);
		
		#$qery['nontexhingstaffDetails'];
		
		
		if( $qery['nontexhingstaffDetails']!='0')
		{
	...
	//editnonteachingstaffdetails starts here
	
	public function editnonteachingstaffdetails()
	{
		$this->load->view(HEADER);
		...
		if( $this->uri->segment('2')>0 )
		{
			...
			$cond['sfd.StaffDetailId'] = $this->uri->segment('2');
			
			$data['Details'] = $this->Commonmodel->getnonteachingdetails($cond);
			...
			if($data['Details']!='0')	
			{
				if( $this->input->post('EditNonTeachingDetails') )
				{
					...
					if( $this->form_validation->run() === false)
						$this->load->view('Admin/edit-nonteaching-staff-details',$data);
					else
					{
						...
						$setdata['StaffId'] 		= $this->input->post('StaffId');
						$setdata['ContactNumber'] 	= $this->input->post('ContactNumber');
						$setdata['ContactAddress'] 	= $this->input->post('ContactAddress');
						$setdata['LastUpdated'] 	= time();
						
						$cond = array();
						
						$cond['StaffDetailId'] = 	$this->uri->segment('2');
						
						if( $this->Commonmodel->updatedata($table,$setdata,$cond)	 )
						{
							
							$msg = "<div class='alert alert-success'>Non teaching staff details updated successfully</div>";
							$this->session->set_flashdata('NtsdetailsSuccess',$msg);
						}
						else
						{
							$msg = "<div class='alert alert-warning'>Unable to update Non teaching staff details</div>";
							$this->session->set_flashdata('failedtoaddNtsdetails',$msg);
						}
						
						redirect(base_url('edit-nonteaching-staff-details/'.$this->uri->segment(2)) );
				...
				{
					$data['routeto'] = 'view-nonteaching-staff-details';
					$this->load->view('Admin/pagenotfound',$data);	
				}
	...
	//editnonteachingstaffdetails ends here
