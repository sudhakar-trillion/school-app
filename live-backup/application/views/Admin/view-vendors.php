...
<section id="main-content">
 ...
 <div class="row">
									...
									if($this->uri->segment(2)) { $slno = ($this->uri->segment(2)-1)*$perpage;	}
                                    		foreach( $vendorsDetails['Vendors']->result() as $vendor)
											{
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                    <a  href="<?PHP echo base_url('edit-vendor')."/".$vendor->VendorId ?>"><i class="fa fa-edit" style="font-size:22px"></i> </a>
                                                    
                                                    <?PHP if($vendor->Status=="Active")
													{
														?>
                                                    <a title="Change status" style="cursor:pointer" class="inactivate_activate" stats = "<?PHP if($vendor->Status=="Active") echo "Inactive"; else echo "Active"; ?>" id="<?PHP echo $vendor->VendorId; ?>"><i class="fa fa-times" style="font-size: 17px;
    color: #fff;
    margin-left: 10px;
    border-radius: 36%;
    background: red;
    ...
    <?PHP 
			...
			{
				?>
                <a title="Change status" style="cursor:pointer" class="inactivate_activate" stats = "<?PHP if($vendor->Status=="Active") echo "Inactive"; else echo "Active"; ?>" id="<?PHP echo $vendor->VendorId; ?>"><i class="fa fa-check" style="font-size: 17px;
    color: #fff;
    margin-left: 10px;
    border-radius: 36%;
    background:#57a602;
